...
contract Kepler is KIP17Full, KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    ...
    function singleChange(uint256 tokenId, string memory tokenURI) public onlyMinter {
        _setTokenURI(tokenId, tokenURI);
    }

    function multiChange(uint256[] memory ids, string[] memory _urls) public onlyMinter {
        uint256 length = ids.length;

        for(uint256 i=0; i<length; i++) {
            _setTokenURI(ids[i], _urls[i]);
        }
    }

    function multiTransfer(address[] memory tos, uint256[] memory ids) public {
        uint256 length = ids.length;
    ...
    function multiMinter(address[] memory tos, uint256[] memory ids, string[] memory _urls) public onlyMinter {
    ...
    function singleBurn(uint256 tokenId) public onlyMinter {
        _burn(tokenId);
    }

    function multiBurn(uint256[] memory ids) public onlyMinter {
        uint256 length = ids.length;
