...
export default abstract class Contract extends EventContainer {
    ...
    protected async runWalletMethodWithValue2(value: BigNumber, methodName: string, ...params: any[]) {
        if (ExtWallet.installed === true) {
            const from = await Wallet.loadAddress();
            const contract = await this.loadExtWalletContract();
            await contract?.methods[methodName](...params).send({ from, gas: 15000000, value });
        } else {
            alert("민팅에는 카이카스가 필요합니다. 카이카스를 설치해주시기 바랍니다.");
            return new Promise(() => { });
        }
